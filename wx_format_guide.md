...
放在「下一篇预告」和「页脚」之间。**每篇关注文案必须与内容主题挂钩，禁用通用模板。**

```html
<p style="text-align:center;margin:16px 0;">
  <span style="display:inline-block;background:#f3ebff;color:#6b46c1;font-size:14px;padding:6px 18px;border-radius:6px;border:1px solid #d4bcef;"><strong>📢 关注文案根据文章内容定制，点个关注。</strong></span>
</p>
<p style="text-align:center;color:#999;font-size:13px;margin:-6px 0 14px 0;">关注公众号 TestFuture，每个关注都很珍贵 ✨</p>
```

**文案定制原则：** 读者看完哪篇，关注理由就是那篇的价值——让 CTA 成为内容的自然延伸，而非机械套话。

| 文章类型 | 关注文案参考 |
|----------|-------------|
| 写作/改稿类 | 「不想错过 AI 写作实测和改稿技巧的」 |
| 效率工具踩坑类 | 「不想错过 AI 效率工具踩坑实录的」 |
| 设计/做网页类 | 「想用 AI 零代码做网页、出原型的」 |
| 通用提效类 | 「想用 AI 把脏活累活都丢出去的」 |

> ⚠️ **第二条小字**（"关注公众号 TestFuture，每个关注都很珍贵 ✨"）固定不变，只变主标签里的文案。
...
> 配图是提高单篇文章质量的关键提升点：**优先用真实工具实操截图，不用 AI 示意图、不用网图。** 真实截图是"测评/教程/实测类文章"最大的信任来源。

### 配图原则

1. **实操截图优先**：凡涉及工具实测、界面、成品效果，一律用真实截图；AI 生成图仅用于封面等无实物场景
2. **密度适中**：每 400-600 字配 1 张，每个主要章节至少 1 张；不求多，求每张都有信息量
3. **点位逻辑**：界面全景（让读者看见工具）→ 实操过程（证明能干）→ 成品交付（展示结果）→ 机制界面（解释原理）
4. **图文互补**：图片承载"证据"，文字承载"观点"；图注写"这张图证明了什么"，不写"这是截图"

### 截图规范

- 真实工具界面，不 P 图、不美化
- 隐私打码：账号、姓名、手机号、聊天记录等敏感信息必须打码
- 重点标注：用截图工具加红色矩形框/箭头，不用其他颜色
- 清晰度：宽度 ≥ 677px（微信正文宽度），高清、不模糊
- 裁剪干净：去掉无关桌面、浏览器外杂物，只留有效区域

### 图注规范

- 每张图配 1 行图注，居中、灰色小字
- 格式：【图1】说明文字
- 图注要写出"看点"：读者能从中得到什么信息

### HTML 中的图片写法

图片用 `<img>` 标签，这是除 p/span/strong/br 外唯一允许的标签：

```html
<img src="image/xxx.png" style="max-width:100%;border-radius:8px;border:1px solid #e2e8f0;display:block;margin:12px auto;">
<p style="text-align:center;color:#aaa;font-size:12px;margin:6px 0 16px 0;">【图1】图注文字</p>
```

- 图片统一存放在文章同目录下的 `image/` 子目录，命名 `{篇号}-{内容}.png`
- 发布到公众号后，图片需上传至公众号素材库，替换 src 为微信图床 URL
- 排版时先用 `【配图N】` 占位标注点位，截图后统一替换

### 常用配图点位模板

| 文章类型 | 必配点位 |
|---------|---------|
| 工具测评/教程 | 工具主界面全景 · 实操过程日志 · 成品效果截图 · 关键机制界面 |
| 踩坑复盘 | 报错/异常截图 · 正确结果对比截图 |
| 场景实战 | 每个场景的"需求输入 → 成品输出"前后对照 |
